...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate table of contents for README.md file."
    )
    parser.add_argument("-i", "--input", help="input file name", required=False)
    parser.add_argument("-o", "--output", help="output file name", required=False)
    parser.add_argument("-d", "--depth", help="depth of headers", required=False)
    parser.add_argument(
        "-t", "--table-of-contents", help="table of contents name", required=False
    )
...
def find_headers_html(file_contents: str, depth: int = 1) -> List[str]:
    ...
    headers = [
        header.replace(f"<h{depth}>", "").replace(f"</h{depth}>", "")
...
def remove_table_of_contents(file_contents: str) -> str:
    ...
    file_contents = file_contents.replace(file_contents[start:stop], "")
...
def main():
    ...
    table_of_contents = generate_table_of_contents(
        file_contents, config.table_of_contents_name, config.depth
    )
